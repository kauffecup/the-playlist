...
const getAll = async (limit, label, func) => {
  ...
  const requestParams = Array.from(new Array(arrayLength)).map((_, i) => i * limit);
...
const getThisWeeksAlbumsAndSingles = async () => {
  ...
  const albums = await getAll(20, 'albums', async ({ limit, offset }) => {
    const { body: { albums: { items: partialAlbums, total } } } = await spotify.searchAlbums('tag:new', { limit, offset });
    const { body: { albums: items } } = await spotify.getAlbums(partialAlbums.map(({ id }) => id));
    return { items, total };
  });
...
const replacePlaylistWithAlbumTracks = async (id, playlistId, playlistName, albums) => {
  ...
  const initialBatch = take(trackUris, maxUris);
  bar.tick(initialBatch.length);
  trackUris = trackUris.slice(maxUris);
  await spotify.replaceTracksInPlaylist(id, playlistId, initialBatch);

  while (trackUris.length) {
    const batch = take(trackUris, maxUris);
 ...
 */
const formatAlbums = albums => albums.map(({
  artists,
  name,
  popularity,
  release_date,
}) => ({
  artists: artists.map(({ name: artistName }) => artistName).join(', '),
  date: release_date,
  name,
  popularity,
}));
